...
class DBFactory:
    ...
    def _vary_datetime_range(
        self, basetime: datetime, hours_range: float | None = None
    ) -> str:
    ...
    def _insert(
        self, table_name: str, data: dict, *, commit: bool, or_replace: bool = False
    ):
    ...
    def create_individual_player_task(
        self, overrides: dict | None = None, commit: bool = True
    ) -> int:
        now = datetime.now(timezone.utc).isoformat()
        defaults = {
            "run_id": None,
            "file_path": f"/tmp/player_mock_{now}.parquet",
            "status": "pending",
            "attempts": 0,
            "started_at": None,
            "finished_at": None,
    ...
    def create_individual_mastery_task(
        self, overrides: dict | None = None, commit: bool = True
    ) -> int:
        player_id = self.faker.uuid4()
        defaults = {
            "run_id": None,
            "file_path": f"/tmp/mastery_{player_id}.parquet",
            "player_id": player_id,
            "status": "pending",
            "attempts": 0,
            "started_at": None,
    ...
    def create_individual_players_recorded(
        self, overrides: dict | None = None, commit: bool = True
    ) -> str:
    ...
    def create_mock_run(
        ...
        patch: str = "vtest",
        tier_variation: list[str] | None = None,
        division_variation: list[str] | None = None,
        run_status_variation: list[str] | None = None,
        last_heartbeat_variation: float = 1.5,  # Hours
        run_started_at_variation: int = 2,
        player_task_status_variation: list[str] | None = None,
        mastery_task_status_variation: list[str] | None = None,
        ...
        if division_variation is None:
            division_variation = ["I", "II", "III", "IV"]
        if run_status_variation is None:
            run_status_variation = ["failed", "success", "running"]
        if player_task_status_variation is None:
            player_task_status_variation = [
                "success",
                "failed",
                "pending",
                "in_progress",
            ]
        if mastery_task_status_variation is None:
            mastery_task_status_variation = [
                "success",
                "failed",
                "pending",
                "in_progress",
        ...
        started = self._vary_datetime_range(
            datetime.now(timezone.utc), run_started_at_variation
        )
        run_values = {
            "pipeline_name": "mock_run",
            "started_at": started,
            "last_heartbeat": self._vary_datetime_range(
                datetime.now(timezone.utc), last_heartbeat_variation
            ),
            "finished_at": (
                (
                    datetime.fromisoformat(started)
                    + timedelta(seconds=self.faker.random.uniform(240, 600))
                ).isoformat()
        ...
        run_id = self.create_individual_run(
            {**run_values, **run_override}, commit=False
        )
        ...
        pt_started = self._vary_datetime_range(
            datetime.now(timezone.utc), started_at_variation
        )
        player_task_values = {
            "run_id": run_id,
            "file_path": f"/tmp/player_mock_{run_id}.parquet",
            "status": pt_status,
            "attempts": self._vary_int(attempts_variation, 0),
            "started_at": pt_started,
            "finished_at": (
                (
                    datetime.fromisoformat(pt_started)
                    + timedelta(milliseconds=self.faker.random.uniform(10, 1500))
                ).isoformat()
        ...
        for _ in range(players_per_task):
            ...
            mt_started = self._vary_datetime_range(
                datetime.now(timezone.utc), started_at_variation
            )
            player_id = self.faker.uuid4()
            mastery_values = {
                ...
                "finished_at": (
                    (
                        datetime.fromisoformat(mt_started)
                        + timedelta(milliseconds=self.faker.random.uniform(10, 1500))
                    ).isoformat()
            ...
            paths = [f"/tmp/player_{player_id}.parquet"]
            ...
            for fake in range(self._vary_int(tasks_per_player_recorded_variation, 1)):
                recorded_player_task_ids.insert(
                    0, -(fake + 1)
                )  # negative: never collides with real autoincrement ids
                paths.insert(0, f"/tmp/player_{player_id}_{fake}.parquet")
...
class FakeAPIClient:
    ...
    def __init__(
        ...
        masteries_payload=None,
        patch: str = "15.1",
        status_code: int = 200,
    ):
        self.players_payload = players_payload if players_payload is not None else []
        self.masteries_payload = (
            masteries_payload if masteries_payload is not None else []
        )
        self.patch = patch
        self.status_code = status_code
...
def assert_valid_player_file(
    ...
    *,
    ...
    actual_puuids = set(table["puuid"].to_pylist())
