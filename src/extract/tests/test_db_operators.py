...
def test_run_lifecycle_operators(mock_conn):
	...
	finished = mock_conn.execute("SELECT status, finished_at, error_message FROM runs WHERE run_id = ?", (run_id,)).fetchone()
...
def test_player_task_lifecycle_operators(mock_conn):
	...
	task = mock_conn.execute("SELECT status, attempts, started_at FROM player_tasks WHERE task_id = ?", (task_id,)).fetchone()
	assert task["status"] == "in_progress"
	assert task["attempts"] == 1
	assert task["started_at"] is not None

	db.update_player_task(task_id, "success", file_path="/tmp/player_snapshot.json", conn=mock_conn)
	task = mock_conn.execute("SELECT status, finished_at, file_path FROM player_tasks WHERE task_id = ?", (task_id,)).fetchone()
...
def test_player_record_and_mastery_operators(mock_conn):
	...
	db.add_player_records(player_id, "/tmp/p1.json", player_task_id, "na1", "RANKED_SOLO_5x5", "GOLD", "I", "14.1", conn=mock_conn)
	db.add_player_records(player_id, "/tmp/p2.json", player_task_id + 1, "na1", "RANKED_SOLO_5x5", "PLATINUM", "II", "14.2", conn=mock_conn)
