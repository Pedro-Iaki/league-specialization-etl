...
class SteadyClient:
    ...
    def get(self, url, **kwargs):
        return util.FakeResponse(
            [
                util.create_player_payload("steady-p1"),
                util.create_player_payload("steady-p2"),
            ]
        )
...
class _TierClient:
    ...
    def get(self, url, **kwargs):
        ...
        return util.FakeResponse(
            [
                util.create_player_payload(f"{tier_key}-p1"),
                util.create_player_payload(f"{tier_key}-p2"),
            ]
        )


def test_concurrent_get_players_runs_do_not_corrupt_files_or_database(
    tmp_path, mock_db, monkeypatch
):
    ...
    tasks = conn.execute(
        "SELECT status, file_path FROM player_tasks WHERE run_id = ?", (run_id,)
    ).fetchall()
    recorded = {
        row["player_id"]
        for row in conn.execute("SELECT player_id FROM players_recorded").fetchall()
    }
    conn.close()
    ...
    expected_players = {f"{tier.lower()}-p1" for tier in TIERS} | {
        f"{tier.lower()}-p2" for tier in TIERS
    }
    assert recorded == expected_players

    all_files = list(tmp_path.rglob("players_*.parquet"))
...
def test_concurrent_get_masteries_do_not_process_the_same_player_twice(
    tmp_path, mock_db, db_factory, monkeypatch
):
    ...
    client = util.FakeAPIClient(
        masteries_payload=[util.create_mastery_payload("shared", champion_id=1)],
        patch="15.1",
    )
    ...
    mastery_files = list(tmp_path.rglob("masteries_*.parquet"))
...
def test_concurrent_update_mastery_task_matches_records(mock_db, db_factory):
    ...
    for i in range(worker_count):
        ...
        puuid = factory.get_uuid()
        puuids.append(puuid)
        task_id = factory.create_individual_mastery_task(
            {"run_id": run_id, "player_id": puuid, "status": "pending"}
        )
        task_ids.append(task_id)
        factory.create_individual_players_recorded(
            {"player_id": puuid, "mastery_status": "pending"}
        )
    conn.close()
    ...
    def _worker(i: int):
        ...
        mock_db.update_mastery_task(
            ...
            status,
            "15.1",
            file_path=f"/tmp/mastery_{puuid}_{i}.parquet"
            if status == "success"
            else None,
    ...
    conn = mock_db.get_connection()
    tasks = conn.execute(
        "SELECT status, file_path FROM mastery_tasks WHERE task_id IN ({seq})".format(
            seq=",".join(["?"] * len(task_ids))
        ),
        task_ids,
    ).fetchall()
    records = conn.execute(
        "SELECT mastery_status, mastery_path FROM players_recorded WHERE player_id IN ({seq})".format(
            seq=",".join(["?"] * len(puuids))
        ),
        puuids,
    ).fetchall()
