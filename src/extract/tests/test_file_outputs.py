...
def test_get_players_run_writes_file_and_records_players(
...
def test_get_players_run_skips_writing_file_when_no_new_players(
    ...
    client = util.FakeAPIClient(
        players_payload=[util.create_player_payload("p1")], patch="15.1"
    )
    get_players.run(
        run_id,
        api_client=client,
...
def test_get_masteries_run_writes_file_and_updates_status(
    tmp_path, mock_db, db_factory, monkeypatch
):
...
def test_get_masteries_run_marks_task_failed_on_error_response(
    tmp_path, mock_db, db_factory, monkeypatch
):
    monkeypatch.setattr(get_masteries, "OUTPUT_PATH", tmp_path)
    ...
    conn = mock_db.get_connection()
    task = conn.execute(
        "SELECT status FROM mastery_tasks WHERE run_id = ?", (run_id,)
    ).fetchone()
    record = conn.execute(
