...
def test_verify_db_integrity_flags_expected_faulty_records(
    ...
    failed_task_id = factory.create_individual_player_task(
        {
            ...
            "status": "failed",
            "file_path": None,
            "error_message": "simulated player task failure",
        }
    )
    success_mastery_id = factory.create_individual_mastery_task(
        {"run_id": run_id, "status": "success", "player_id": factory.get_uuid()}
    )
    factory.create_individual_mastery_task(
        {
            "run_id": run_id,
            "status": "failed",
            "player_id": factory.get_uuid(),
            "error_message": "simulated mastery task failure",
        }
    )
    ...
    factory.create_individual_players_recorded(
        {
            "player_id": "pending-mastery-player",
            "region": "na1",
            "queue": "RANKED_SOLO_5x5",
            "tier": "GOLD",
            "division": "I",
    ...
    result = verify_integrity.verify_db_integrity(conn)
...
def test_verify_files_integrity_detects_missing_and_duplicated_puuids(
    ...
    missing_masteries_puuids = {
        entry["puuid"] for entry in result["players_missing_masteries"]
    }
    assert missing_masteries_puuids == {"p2"}

    missing_players_puuids = {
        entry["puuid"] for entry in result["masteries_missing_players"]
    }
