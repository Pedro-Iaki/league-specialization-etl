...
def create_failed_run_database(_db_factory, _mock_conn) -> int:
    ...
    factory = _db_factory(_mock_conn)
    failed_run_id = factory.create_individual_run(
    ...
    for pt_status in status_states:  # loop to simulate a player task for each status
        ptask_id = factory.create_individual_player_task(
            {"run_id": failed_run_id, "status": pt_status}
        )

        for id_length in range(
            1, 4
        ):  # loop to simulate players with different number of task ids
            for mt_status in status_states:  # loop to simulate a mastery task for each player task, with the same status
                ...
                player_task_ids = [
                    ptask_id
                ]  # generate the player task ids based on loop count
                for fake in range(id_length - 1):
                    player_task_ids.insert(
                        0, -(fake + 1)
                    )  # negative so no conflict with real autoincrement ids

                factory.create_individual_players_recorded(
                    {
                        "player_task_ids": json.dumps(player_task_ids),
                        "player_id": player_id,
                        "mastery_status": mt_status,
...
def test_failed_run_scenario(db_factory, mock_conn):
    ...
    run_player_tasks = mock_conn.execute(
        "SELECT task_id, status FROM player_tasks WHERE run_id = ?", (failed_run_id,)
    ).fetchall()
    run_mastery_tasks = mock_conn.execute(
        "SELECT task_id, status FROM mastery_tasks WHERE run_id = ?", (failed_run_id,)
    ).fetchall()
    ...
    for task_id, status in run_player_tasks:
        if status != "success":
            ...
            assert (
                len(
                    mock_conn.execute(
                        "SELECT 1 FROM players_recorded WHERE player_task_ids = ?",
                        (f"[{task_id}]",),
                    ).fetchall()
                )
                == 0
    ...
    for task_id, status in run_mastery_tasks:
        if status != "success":
            ...
            assert (
                len(
                    mock_conn.execute(
                        "SELECT 1 FROM players_recorded WHERE mastery_task_id = ? AND mastery_status != 'failed'",
                        (task_id,),
                    ).fetchall()
                )
                == 0
...
def test_cleanup_stale_runs(db_factory, mock_conn, monkeypatch):
    ...
    monkeypatch.setattr(
        db, "cleanup_failed_run", lambda *args, **kwargs: None
    )  # set cleanup_failed_run to nothing

    stale_run_id = factory.create_individual_run(
        {
            "status": "running",
            "last_heartbeat": (
    ...
    active_run_id = factory.create_individual_run(
        {"status": "running", "last_heartbeat": datetime.now(timezone.utc).isoformat()},
        commit=True,
    )

    db.cleanup_stale_runs(conn=mock_conn)

    stale_run_status = mock_conn.execute(
        "SELECT status FROM runs WHERE run_id = ?", (stale_run_id,)
    ).fetchone()[0]
    assert stale_run_status == "failed"

    active_run_status = mock_conn.execute(
        "SELECT status FROM runs WHERE run_id = ?", (active_run_id,)
    ).fetchone()[0]
